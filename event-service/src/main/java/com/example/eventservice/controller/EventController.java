...
public class EventController {
    ...
    public ResponseEntity<EventResponse> createEvent(@Valid @RequestBody EventRequest eventRequest) {
        logger.info("Requête de création d'événement reçue : {}", eventRequest.getTitle());
     ...
     */
    @PutMapping("/{id}")
    public ResponseEntity<EventResponse> updateEvent(@PathVariable Long id, @Valid @RequestBody EventUpdateRequest updateRequest) {
        logger.info("Requête de mise à jour d'événement reçue pour l'ID : {}", id);

        try {
            EventEntity existingEvent = eventService.getEventById(id);
            eventMapper.updateEventEntity(existingEvent, updateRequest);
            EventEntity updatedEvent = eventService.updateEvent(id, existingEvent);
